                            ...
                            <td class="colspan-2 text-center">
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" id="${knjige[i].knjiga_id}">
                                         <li><button  class="dropdown-item" onclick="zelimProcitatiProcitao(this)" type="button">Want to read</button></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
                        ...
                        <th scope="row">${i + 1}</th>
                         ...
                         <td class="colspan-2 text-center">
                             <div class="btn-group">
                                 ...
                                 <ul class="dropdown-menu" id="${knjige[i].knjiga_id}">
                                     <li><button  class="dropdown-item" onclick="zelimProcitatiProcitao(this)" type="button">Read it</button></li>
                                 </ul>
                              </div>
                         </td>
                    </tr>
...
function zelimProcitatiProcitao(btn) {
    ...
    if (btn.innerHTML === 'Read it') {

        procitao = 1
        zeli_procitati = 0
    }
    else if (btn.innerHTML === 'Want to read') {
        ...
        .then(rez => {
            if (rez) {
                ...
                if (procitao && !zeli_procitati) {
                    ...
                    ul.innerHTML = ''
                    ul.innerHTML = `  <li><button  class="dropdown-item" onclick="zelimProcitatiProcitao(this)" type="button">Want to read</button></li>`
                    ...
                    button_mark.innerHTML = ` <i class="fas fa-check  text-warning"></i>
                                 Want to read`
                    ul.innerHTML = ''
                    ul.innerHTML=`<li><button class="dropdown-item" onclick="zelimProcitatiProcitao(this)" type="button">Read it</button></li>`
                ...
                if (par[i].knjiga_id === lista[j].knjiga_id) {
                    ...
                    const tbl = `
                         <tr class="bt-2" id="${par[i].knjiga_id}" >
                              ...
                              <td class="colspan-2"><button class="btn btn-outline-success" onclick="dodajKnjiguUListu(this)"><i class="fas fa-check"> Added</i></button></td>
                         </tr>
                 `
                    kontejner.innerHTML += tbl
                    indexi[br] = i
    ...
    for (var i = 0; i < par.length; i++) {
        ...
        const tbl = `
          <tr class="bt-2" id="${par[i].knjiga_id}" >
               <th scope="row"><img src="/assets/knjige/${par[i].slika}" onclick="window.location='/korisnik/knjiga/${par[i].knjiga_id}'" style=" width: 60px; height: 100px;"></th>
               <td class="colspan-2"><h5><b>${par[i].naziv}</b></h5> <br> <p>by ${par[i].autor}</p></td>
               <td class="colspan-2"><button class="btn bg-success" onclick="dodajKnjiguUListu(this)">Add to list</button></td>
          </tr>
                  `
    ...
    fetch('http://localhost:5000/korisnik/liste/nova-lista/add?id_knj=' + id_knj + '&id_lista=' + id_lista)
        ...
        .then(rez => {
            if (rez) {
                ...
                btn.innerHTML = '<i class="fas fa-check"> Added</i>'
            }
        })
